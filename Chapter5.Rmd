...
new_priors <- c(# Priors for K ~ M + N
                 ...
                 set_prior("normal(0,0.05)", class="b", resp="N"),
...
# Simulate counterfactual
scf <- predict(mdl5H3_b, newdata = simdata, resp="K")
# Does this give same result?
scfN <- predict(mdl5H3_b, newdata = simdata, resp="N")
simdata$N <- scfN[,1]
scfK <- predict(mdl5H3_b, newdata = simdata, resp="K")


ggplot() +
  geom_point(mapping=aes(x=simdat$M,
                        y=scf[,1],
                        color="Counterfactual1")) +
  # geom_point(mapping=aes(x=df$M,
  #                       y=ppd[,1],
  #                       color="Predicted")) +
  geom_point(mapping=aes(x=simdat$M,
                        y=scfK[,1],
                        color="Counterfactual2")) +
  labs(x="log Body Mass", y="kcal per g")
```
