...
samples <- add_epred_draws(newdata=data.frame(pred=unique(frogs$pred),
                                   tank=1,
                                   density=1), 
                           mdl1a) 
...
samples %>% 
  ggplot() +
  geom_density(mapping=aes(x=.epred)) +
  facet_wrap(~pred)
...
samples %>% 
  ...
  ggplot() +
  geom_pointrange(aes(x=pred, y=est, ymin=lwr, ymax=upr)) +
  labs(title="Conditional Effect (tank=1, reference category)")

conditional_effects(mdl1a)
```
...
```{r}
conditional_effects(mdl3a, categorical = TRUE)
```
